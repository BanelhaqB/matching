...
const cleanSitemap2 = async (xml) => {
  return new Promise(async (resolve) => {
    ...
    array.forEach((line) => {
      // if (line.substring(0, 10) === '<loc>https')
      // console.log(line.split('https://yoopies.fr/')[1]);
      // console.log(line.split('https://yoopies.fr/')[1].substring(0, 12));
      if (
        line.substring(0, 10) === '<loc>https' &&
        (line.split('https://yoopies.fr/')[1].substring(0, 13) ===
          'emploi-menage' ||
          line.split('https://yoopies.fr/')[1].substring(0, 22) ===
            'emploi-aide-a-domicile' ||
          line.split('https://yoopies.fr/')[1].substring(0, 22) ===
            'emploi-femme-de-menage' ||
          line.split('https://yoopies.fr/')[1].substring(0, 15) ===
            'femme-de-menage' ||
          line.split('https://yoopies.fr/')[1].substring(0, 6) === 'nounou')
      )
        urls.push({
          url: line.substring(5, line.length - 6),
          id: line.substring(5, line.length - 6).split('/')[
    ...
    await utils.convertToCSV(
      urls,
      `data/yoopies/urls2/urls-yoopies.csv`,
      false
    );
...
const filterURLS2 = async () => {
  const urls = await utils.readCSV(`data/yoopies/urls2/urls-yoopies.csv`, ',');
  console.log(1, urls.length);
  // keep only profil url
  let urlsFiltred = _.filter(urls, (e) => {
    return e.id * 1 > 0;
  });
  console.log(2, urlsFiltred.length);
  // Deleting doublons
  urlsFiltred = _.uniqBy(urlsFiltred, 'id');
  console.log(3, urlsFiltred.length);

  for await (const annonce of urlsFiltred)
    await utils.convertToCSV(
      [annonce],
      `data/yoopies/urls2/urls-yoopies2.csv`,
      false
    );

  console.log(4, urlsFiltred.length);
  console.log(
    `yoopies : URLS are filtred ✅ - (${urlsFiltred.length} unique url)`
  );
  console.log(`----------------------------------------`);
...
const getNew = async () => {
  ...
  const urls = await utils.readCSV(`data/yoopies/urls2/urls-yoopies2.csv`, ',');
  ...
  // await utils.convertToCSV(
  //   newURLS,
  //   `data/yoopies/new2/new-urls-${utils.getDayToday(true)}.csv`,
  //   true
  // );
...
const scrapData = async (url, prenom, nom, tel, id) => {
  return new Promise((resolve, reject) => {
    ...
    const fct = async ($, response, html, config, dataArr) => {
      ...
      await utils.convertToCSV(
        [prof],
        `data/yoopies/new2/new-data-${utils.getDayToday(false)}.csv`,
        false
      );

      await utils.convertToCSV([prof], `data/yoopies/data-yoopies2.csv`, false);
    };
...
const scrapAllNew = async () => {
  ...
  for await (const newUrl of newURLS) {
    if (
      idx >= process.env.INST * 1 * 100000 &&
      idx < (process.env.INST * 1 + 1) * 100000
    ) {
      ...
      try {
        ...
        await utils.convertToCSV(
          [prof],
          `data/yoopies/errors2/errors-yoopies.csv`,
          false
        );
      }
